...
library(ggforce); library(tidyverse); library(extrafont); loadfonts(device='win'); library(lhs); library(fitdistrplus)
```
...
ggplot(dF)+
  geom_histogram(aes(x=dF$S), col='grey25', fill='grey75', binwidth = binWidth)+
...
ggplot(dF)+
  geom_histogram(aes(x=dF$S, y=..density..), col='grey25', fill='grey75', binwidth = binWidth)+
...
ggplot(NULL)+
  ...
  theme_bw(base_family = 'Trebuchet MS')+
  scale_x_continuous(limits=c(minX, maxX), name='Stress or Strength, MPa')+
  scale_y_continuous(name='Probability Density')+
...
ggplot(NULL)+
  ...
  theme_bw(base_family = 'Trebuchet MS')+
  scale_x_continuous(limits=c(minX-100, maxX-100), name='Safety Margin, MPa')+
  scale_y_continuous(name='Probability Density')+
  theme(legend.title = element_blank())+theme(legend.position = 'top')+
  theme(text=element_text(size=9))
